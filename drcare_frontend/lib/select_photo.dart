...
class _ChoosePhotoState extends State<ChoosePhoto> {
  ...
  gettranslatedImage() async {
    var request =
        http.MultipartRequest('POST', Uri.parse(routes[2]['translation']));
    ...
    // var finalresult =
    //     json.decode(String.fromCharCodes(await res.stream.toBytes()));
    var translatedImage = String.fromCharCodes(await res.stream.toBytes());
                              ...
                              child: MaterialButton(
                                onPressed: () async {
                                  if (image == null) {
                                    ...
                                    } else if (widget.person.isLeukemiaTest) {
                                      setState(() => loading = true);
                                      leukemiaResult =
                                    ...
                                    } else if (widget.person.isTranslation) {
                                      setState(() => loading = true);
                                      var translatedImagelink =
                                          await gettranslatedImage();
                                      if (translatedImagelink == null) {
                                        setState(() => loading = false);
                                      }
                                      print(translatedImagelink);

                                      APIHandler().enterResult(
                                          person: widget.person,
                                          finalfinal: translatedImagelink);
                                      Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (context) =>
