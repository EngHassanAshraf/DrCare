...
class APIHandler {
  ...
  sendBasicInfo(String gender, String age) async {
    ...
    print("Basic Info Status Code:: ${res.request}");
  }
  ...
  Person enterResult({
    ...
    } else if (person.isTranslation) {
      // print("1from apiHandler ${(person.transltedImage)}");
      // print(
      //     "2from apiHandler ${json.decode(person.transltedImage).runtimeType}");
      // print(
      //     "from apiHandler ${(json.decode(person.transltedImage)['imagePath'])}");
      person.transltedImage = finalfinal;
    }
